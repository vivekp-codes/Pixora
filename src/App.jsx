...
const GithubIcon = (props) => (
  <svg viewBox="0 0 24 24" width="15" height="15" fill="currentColor" aria-hidden="true" {...props}>
    <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 2.834 2.807 2.304 3.492 1.76.097-1.122.428-1.887.782-2.322-2.91-.318-5.969-1.455-5.969-6.486 0-1.435.513-2.607 1.354-3.522-.135-.331-.587-1.667.131-3.468 0 0 1.097-.36 3.592 1.34.999-.276 2.069-.418 3.134-.42 1.065.002 2.136.144 3.138.42 2.493-1.7 3.589-1.34 3.589-1.34.72 1.801.264 3.137.129 3.468.843.915 1.352 2.087 1.352 3.522 0 5.042-3.063 6.165-5.98 6.479.452.39.86 1.152.86 2.325v3.428c0 .291.106.632.65.525 4.78-1.574 8.213-6.086 8.213-11.386 0-6.627-5.373-12-12-12z" />
  </svg>
)
    ...
    <footer className="site-footer">
      <div className="footer-top">
        <div className="footer-brand">
          <img src="/Plogo.png" alt="Pixora" className="footer-logo" />
          <p className="footer-tag">Create stunning AI art from a single prompt. Fast, fun, and endlessly creative.</p>
        ...
        <div className="footer-cols">
          <div className="footer-col">
            ...
            <button onClick={() => onNavigate('creations')}>Creations</button>
      ...
      <div className="footer-bottom">
        <span>© {new Date().getFullYear()} Pixora Labs. All rights reserved.</span>
        <span>Made with AI · Powered by imagination</span>
      </div>
    </footer>
  )
}

function TrendingRow({ images, reverse }) {
  const doubled = [...images, ...images]
  return (
    <div className="trend-row">
      <div className={`trend-track${reverse ? ' reverse' : ''}`}>
        {doubled.map((src, i) => (
...
function TrendingTopNow() {
  const row1 = Array.from({ length: 9 }, (_, i) => `/Images/img-ast/img${i + 1}.jpg`)
  const row2 = Array.from({ length: 9 }, (_, i) => `/Images/img-ast/img${i + 10}.jpg`)
  return (
    <section className="trending">
      <div className="section-head">
        <h2>Trending top now</h2>
      </div>
      <TrendingRow images={row1} />
      <TrendingRow images={row2} reverse />
    </section>
  )
}
